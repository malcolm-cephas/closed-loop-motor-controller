"""
Phase 7 — 150 RPM Physical Closed-Loop Speed Validation Script.

Executes 3 physical repeatability runs at 150 RPM target (Kp=0.5, Ki=15.0, 3-sample MA),
runs HIL comparison simulation, logs telemetry, and generates plots.
"""
import os
import sys
import math
...
def run_phase7_validation():
    ...
    print("=== PHASE 7 — 150 RPM CLOSED-LOOP TEST EXECUTION ===")
    ...
    for run_idx in range(1, 4):
        ...
        # 3. Closed-loop start (target_rpm = 150)
        ...
        for t_step in range(500):
            ...
            run_data.append({
                ...
                "raw_rpm": round(raw_rpm, 2),
                "filtered_rpm": round(filt_rpm, 2),
        ...
        # Performance Metrics Calculations for 150 RPM target
        times = [d["timestamp_ms"] / 1000.0 for d in run_data]
        raw_rpms = [d["raw_rpm"] for d in run_data]
        filt_rpms = [d["filtered_rpm"] for d in run_data]
        duties = [d["pwm_duty"] for d in run_data]
        currents = [d["motor_current"] for d in run_data]

        # Rise time (10% to 90% of 150 RPM -> 15.0 to 135.0 RPM)
        t_10 = next((t for t, r in zip(times, filt_rpms) if r >= 15.0), 0.0)
        t_90 = next((t for t, r in zip(times, filt_rpms) if r >= 135.0), 0.0)
        rise_ms = (t_90 - t_10) * 1000.0

        peak_rpm = max(filt_rpms)
        overshoot_pct = (max(0.0, peak_rpm - 150.0) / 150.0) * 100.0

        # Settling time (±2% band -> 147.0 to 153.0 RPM)
        # Using 5-sample moving window to filter discrete quantization noise
        window = 5
        sma_rpms = [
        ...
        sse = sum(150.0 - r for r in ss_filt) / len(ss_filt)
    ...
    # Save CSV
    csv_path = "analysis/data/closed_loop_150rpm.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "run", "timestamp_ms", "target_rpm", "raw_rpm", "filtered_rpm", "error_rpm",
            "pi_output", "pwm_duty", "motor_current", "encoder_count", "state", "fault_flags"
        ])
        writer.writeheader()
        for d in runs_telemetry:
            # write row with exact field names requested
            row = dict(d)
            del row["run"]
            writer.writerow(row)
    print(f"Saved telemetry to {csv_path}")

    # Generate 150 RPM Physical Response Plot
    try:
        import matplotlib
        matplotlib.use('Agg')
        ...
        colors = ['b', 'g', 'r']
        for run_idx in range(1, 4):
            run_d = [runs_telemetry[i] for i in range(len(runs_telemetry)) if runs_telemetry[i]["run"] == run_idx]
            times = [d["timestamp_ms"] / 1000.0 for d in run_d]
            raw = [d["raw_rpm"] for d in run_d]
            filt = [d["filtered_rpm"] for d in run_d]
            ...
            ax1.plot(times, raw, color=c, alpha=0.3, label=f'Run {run_idx} Raw RPM' if run_idx==1 else None)
            ax1.plot(times, filt, color=c, label=f'Run {run_idx} Filtered RPM (3-sample MA)')
            ax2.plot(times, duties, color=c, label=f'Run {run_idx} PWM Duty %')
            ax3.plot(times, currents, color=c, label=f'Run {run_idx} Current (A)')

        ax1.set_title("Physical Closed-Loop 150 RPM Speed Test (Kp=0.5, Ki=15.0, 3-sample MA)")
        ...
        plt.tight_layout()
        plot_path = "analysis/plots/closed_loop_150rpm.png"
    ...
    print("\n=== HIL VS PHYSICAL 150 RPM SIMULATION COMPARISON ===")
    ...
    # HIL Metrics
    t_10_h = next((t for t, r in zip(hil_times, hil_rpms) if r >= 15.0), 0.0)
    t_90_h = next((t for t, r in zip(hil_times, hil_rpms) if r >= 135.0), 0.0)
    rise_ms_h = (t_90_h - t_10_h) * 1000.0
    peak_h = max(hil_rpms)
    overshoot_h = (max(0.0, peak_h - 150.0) / 150.0) * 100.0
    settling_ms_h = 0.0
    for i in range(len(hil_rpms)):
        if all(147.0 <= r <= 153.0 for r in hil_rpms[i:]):
            settling_ms_h = hil_times[i] * 1000.0
            break
    ss_rpms_h = [r for t, r in zip(hil_times, hil_rpms) if t >= 3.0]
    sse_h = sum(150.0 - r for r in ss_rpms_h) / len(ss_rpms_h)
    ...
    try:
        ...
        phys_run1 = [d for d in runs_telemetry if d["run"] == 1]
        phys_times = [d["timestamp_ms"] / 1000.0 for d in phys_run1]
        phys_filt = [d["filtered_rpm"] for d in phys_run1]
        phys_duties = [d["pwm_duty"] for d in phys_run1]
        phys_currents = [d["motor_current"] for d in phys_run1]
        ...
        ax1.set_title("HIL vs Physical Comparison — 150 RPM Closed-Loop Step Response")
        ax1.set_ylabel("Speed (RPM)")
        ax1.legend()
        ax1.grid(True)
        ...
        plt.tight_layout()
        plt.savefig("analysis/plots/hil_vs_physical_150rpm.png", dpi=150)
        plt.close()
        print("Saved plot: analysis/plots/hil_vs_physical_150rpm.png")
    except Exception as e:
        print(f"HIL comparison plot failed: {e}")

    print("\n--- PHASE 7 PERFORMANCE SUMMARY ---")
    for m in run_metrics:
        print(f"Run {m['run']}: Rise={m['rise_ms']}ms, Peak={m['peak_rpm']}RPM, Overshoot={m['overshoot_pct']}%, Settling={m['settling_ms']}ms, SSE={m['sse']}RPM, SSVarRaw={m['ss_var_raw']}RPM, SSVarFilt={m['ss_var_filt']}RPM, StartupCurr={m['startup_current']}A, MaxCurr={m['max_current']}A, SSCurr={m['ss_current']}A, MaxPWM={m['max_pwm']}%, FinalPWM={m['final_pwm']}%")
